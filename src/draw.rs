...
fn draw_tabs(f: &mut Frame, top_bar: &[Rect], app: &App) {
    ...
    let titles: Vec<Line> = TABS
        ...
        .map(|(i, t)| {
            let icon = match i {
                0 => crate::symbols::tab_scoreboard(),
                1 => crate::symbols::tab_gameday(),
                2 => crate::symbols::tab_stats(),
                3 => crate::symbols::tab_standings(),
                _ => "",
            };
...
fn draw_scoreboard(f: &mut Frame, rect: Rect, app: &mut App) {
    ...
    if let Some(matchup) = app.state.schedule.get_probable_pitchers_opt() {
        ...
        let weather_info = app.state.gameday.game.weather.as_ref().and_then(|w| {
            ...
            Some((crate::symbols::format_weather(condition, temp), temp_val))
        });
            ...
            let block = Block::default()
                ...
                .title_bottom(
                    Line::from(Span::styled(
                        format!(" {title} "),
                        Style::default().fg(palette::temp_color(*temp_val)),
                    ))
                    .alignment(Alignment::Right),
                );
